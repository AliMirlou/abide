...
def collect_and_download(derivative, pipeline, strategy, out_dir,
	...
	try:
		...
		raise Exception('Unable to extract header information from the pheno file: %s\n'
		                'Header should have pheno info: %s\nError: %s' % (s3_pheno_path, str(header), exc))
	...
	for pheno_row in pheno_list[1:]:
		...
		if site is not None and site.lower() != row_site.lower():
			continue
		# Test age range
		if less_than > row_age > greater_than:
	...
	for path_idx, s3_path in enumerate(s3_paths):
		...
		try:
			if not os.path.exists(download_file):
				...
				download_with_resume(s3_path, download_file)
...
if __name__ == '__main__':
	...
	collect_and_download(derivative, pipeline, strategy, out_dir, less_than, greater_than, site, sex)
